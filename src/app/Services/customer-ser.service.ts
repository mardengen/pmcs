...
export class CustomerSerService {
  ...
  getHeroNo404<Data>(id: number): Observable<OPEC_PRTYPE_BUYER> {
    const heroesurl="";
    const url = `${this.heroesUrl1}/?id=${id}`;
  ...
  addHero (hero: Customer): Observable<Customer> {
    return this.http.post<Customer>(this.heroesUrl1, hero, httpOptions);
    // .pipe(
    //   tap((hero: Customer) => this.log(`added hero w/ id=${hero.CustomerNo}`)),//.id
  ...
  updateHero (hero: Customer): Observable<any> {
    return this.http.put(this.heroesUrl3, hero, httpOptions);
    // .pipe(
